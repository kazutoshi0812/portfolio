...
// ドロワーメニュークローズ
$(function(){
  $(".header__nav--item a, .header__dropdown a").click(function () {//ナビゲーションのリンクがクリックされたら
...
$(function() {
  ...
  // ドロップダウンボタンがクリックされた時の処理
  $dropdownButton.on('click', function(event) {
      event.preventDefault(); // デフォルトのリンク動作をキャンセル
      $dropdownMenu.toggleClass('show'); // ドロップダウンメニューを表示/非表示
  });
  
  // ドロップダウンメニュー外がクリックされたらメニューを閉じる
  $(document).on('click', function(event) {
      if (!$(event.target).closest('.header__nav--item--dropdown').length) {
